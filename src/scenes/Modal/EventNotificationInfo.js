...
export default class EventNotificationInfo extends Component {
    ...
    render() {
        const { screenWidth, screenHeight, screenHeightWithHeader, history, isVisible, setVisible, title, description } = this.props;
        const { stretch, btnStyle, btnContainer, border, circleView, image, caretImage } = styles;
                ...
                <WView dial={5} style={{ alignItems: 'stretch' }}>
                    <ScrollView contentContainerStyle={[{ minWidth: screenWidth, minHeight: screenHeight, justifyContent: "center", backgroundColor: "rgba(0, 0, 0, 0.7)" }, stretch]}>
                        <WView dial={5} padding={[10, 0, 0, 0]} style={[stretch]} backgroundColor={Palette.white}>
                            ...
                            <WText color={Palette.black} fontSize={15} fontFamily={"Muli-Bold"} padding={[5, 5]}>{title}</WText>
                            <WText color={Palette.border_color} fontSize={14} lines={100000} padding={[5, 5]}>{description}</WText>
